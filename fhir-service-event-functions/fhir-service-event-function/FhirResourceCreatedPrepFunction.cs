    ...
    public class FhirResourceCreatedPrepFunction
    {
            ...
            ILogger log)
        {
            ...
            try
            {
                ...
                {
                    // get auth token
                    string token = await GetFhirServerToken(configuration, client);
        ...
        private async Task<string> GetFhirServerToken(IConfiguration configuration, HttpClient httpClient)
        {
            string token;
