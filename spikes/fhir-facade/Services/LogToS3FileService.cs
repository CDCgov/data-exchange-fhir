...
namespace OneCDPFHIRFacade.Services
{
    ...
    {
        Task<IResult> SaveResourceToS3(IAmazonS3 s3Client, string bucketName, string folderName, string fileName, List<string> content, string requestId);
    }
    public class LogToS3FileService : ILogToS3FileService
    {
        public async Task<IResult> SaveResourceToS3(IAmazonS3 s3Client, string s3BucketName, string folderName, string fileName, List<string> logs, string requestId)
        {
